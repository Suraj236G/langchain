...
class SandboxIntegrationTests(BaseStandardTests):
    ...
    def test_grep_across_nested_directories(
        ...
        sandbox_backend.execute(f"mkdir -p {_quote(base_dir + '/sub1/sub2')}")  # nosemgrep: sqlalchemy-execute-raw-query  # not SQLAlchemy; shell path is sanitized via shlex.quote
    ...
    def test_grep_with_globstar_include_pattern(
        ...
        sandbox_backend.execute(f"mkdir -p {_quote(base_dir + '/a/b')}")  # nosemgrep: sqlalchemy-execute-raw-query  # not SQLAlchemy; shell path is sanitized via shlex.quote
